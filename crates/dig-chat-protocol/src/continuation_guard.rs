...
//! repo's own established idiom, not a rival shape imported from elsewhere. This is one
//! of the smallest crates in the workspace (three source files, ahead of only
//! `dig-runtime`'s one); the floor below is sized for that rather than copied from a
//! larger sibling.
#![cfg(test)]
...
/// This crate has three source files at the time of writing; a scan that reads zero (a
...
const MIN_FILES_SCANNED: usize = 2;
