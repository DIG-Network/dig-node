...
// ---------------------------------------------------------------------------
// Drawing a distinct, independent sample (chia-query 0.10.0 adoption, dig_ecosystem#2904)
// ---------------------------------------------------------------------------
...
async fn a_co_resident_node_no_longer_consumes_the_probe_budget() {
    ...
    let sample =
        super::assemble_distinct_sample(target, super::MAX_PROBE_ATTEMPTS, |exclude| async {
...
async fn priority_peers_are_never_counted_as_independent_voices() {
    ...
    let sample = super::assemble_distinct_sample(4, super::MAX_PROBE_ATTEMPTS, |exclude| async {
        dialler.draw(&exclude)
    })
    .await;
...
async fn a_dialler_that_ignores_exclusions_exhausts_the_bound_without_hanging() {
    ...
    let sample =
        super::assemble_distinct_sample(quorum::QUORUM_DIAL_WIDE, super::MAX_PROBE_ATTEMPTS, |_| {
