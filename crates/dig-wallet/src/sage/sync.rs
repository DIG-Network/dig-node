        ...
        match message.msg_type {
            ...
            ProtocolMessageTypes::NewPeakWallet => {
                if let Ok(peak) = decode::<NewPeakWallet>(&message) {
                    ...
                    if state.peak_height.is_some_and(|current| peak.height < current) {
                        tracing::warn!(
                            claimed = peak.height,
...
mod tests {
    ...
    async fn coin_state_update_reorg_rolls_back_then_applies() {
        ...
        apply_coin_states(&db, &[state(coin(1, 9, 5), Some(10), Some(30))], &subscribed_owned())
            .await
            .unwrap();
    ...
    async fn forward_update_advances_peak_without_rollback() {
        let db = WalletDb::open_in_memory().await.unwrap();
        apply_coin_states(&db, &[state(coin(1, 9, 5), Some(10), None)], &subscribed_owned())
            .await
            .unwrap();
        db.set_peak(40, "aa").await.unwrap();
        let update = CoinStateUpdate {
    ...
    async fn funded_and_synced() -> WalletDb {
        ...
        apply_coin_states(
            &db,
            &[state(coin(1, OWNED, 6_000_000_000_000), Some(5_999_990), None)],
            &subscribed_owned(),
        )
        .await
        .unwrap();
    ...
    async fn a_reorg_deeper_than_the_bound_is_refused_and_the_replica_is_untouched() {
        ...
        assert!(
            matches!(err, SyncError::ForkTooDeep { depth: 6_000_000, max: MAX_REORG_DEPTH }),
            "got {err:?}"
        );
        assert_eq!(
    ...
    async fn run_update_loop_runs_attribution_when_attributor_present() {
        ...
        run_update_loop(&db, receiver, &events, Some(&attributor), &subscribed_owned())
            .await
            .unwrap();
