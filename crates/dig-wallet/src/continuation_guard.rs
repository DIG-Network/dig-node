...
//! repo's own established idiom, not a rival shape imported from elsewhere. This crate
//! ships its OWN `dig-wallet` CLI binary (`src/main.rs`) beside its library surface, so
//! it is not merely prophylactic library-doc coverage -- it is a second, independent
//! user-facing CLI surface in this repo that nothing scanned until now.
#![cfg(test)]
...
/// No fixed-width fixture in this crate is known to need a line-range exemption yet.
const EXCLUDED_LINE_RANGES: &[(&str, u32, u32)] = &[];

/// No CLI banner builder using deliberate `\n`-plus-column alignment has been found in
/// this crate yet. Add a file here if a future summary/status printer needs it, the
/// same way `dig-node-service` exempts its own banner builders.
...
/// This crate has dozens of source files (42 at the time of writing); a scan that reads
...
/// succeed.
const MIN_FILES_SCANNED: usize = 30;
