...
impl<P: ClaimChainPort, H: DistributorHintSource> ClaimEngine<P, H> {
    ...
    pub async fn run_cycle(&mut self, now: u64) -> Vec<ClaimOutcome> {
        ...
        // RED-PROOF REVERT (scratch/f7-red-proof, not for merge): the F7 cadence gate and
    ...
    async fn evaluate_budget_phase(
        ...
        // RED-PROOF REVERT (scratch/f7-red-proof, not for merge): the write-then-spend
        // persistence is disabled here on purpose, so the pre-fix defect reproduces.
