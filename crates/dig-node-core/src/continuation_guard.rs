//! Test-only guard against the "lost string continuation" defect class (dig_ecosystem#3190).
//!
...
//!
//! Ported from `dig-node-service::continuation_guard` (dig-node#526/#501), which this
//! module mirrors line-for-line apart from the crate-specific constants below -- this
//! repo's own established idiom, not a rival shape imported from elsewhere.
#![cfg(test)]
...
/// Two deliberate alignments the crate-wide scan cannot tell from a lost continuation,
/// because the run they create is real content, not string-literal indentation:
/// - `download.rs`'s `claimed(...)` fixture table column-aligns three trailing
///   comments (`// a coin bonding something else -> Unbonded`); the run before `//`
///   is exempt structurally, but the run BETWEEN the comment's own words (before its
///   `->`) is not, so the whole match-table line needs the exemption.
/// - `net.rs`'s two `println!` debug lines column-align their `label : value`
///   prefixes so the colons line up in terminal output -- the same idiom
///   `dig-node-service`'s `CLI_COLUMN_FILES` exempts, but expressed as plain padding
///   with no literal `\n` for that exemption to key off.
const EXCLUDED_LINE_RANGES: &[(&str, u32, u32)] =
    &[("download.rs", 4595, 4595), ("net.rs", 2259, 2259)];

/// No CLI banner builder lives in this crate (it is the node engine library, not the
/// binary) -- so no file needs the `\n`-plus-alignment exemption `dig-node-service`
/// carries for its summary printers.
const CLI_COLUMN_FILES: &[&str] = &[];

/// A lost continuation always leaves 14-22 spaces (the source's own indentation);
/// ordinary column-alignment padding measured in the sibling crate never exceeds 8. Ten
...
/// This crate has dozens of source files (77 at the time of writing); a scan that reads
/// zero (a wrong `CARGO_MANIFEST_DIR`, a moved `src/`, a walk that silently matched
/// nothing) is a broken guard, not a passing one, and must FAIL rather than vacuously
/// succeed.
