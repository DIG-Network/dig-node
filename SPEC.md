...
- **`synced` and `peak_height` are properties of the tier, and `synced` is MEASURED.** A `"db"`
  ...
  **A money read narrows `is_following` in exactly ONE direction: an UNOBSERVABLE peer tier — no
  ...
  A `"fallback"` answer MUST NOT derive either field from the local DB's state, **regardless of what
  that state is** — the DB neither produced that figure nor bounds its freshness, so its flag and
  peak say nothing about it. Implementations MUST NOT read those two fields outside the tier
  decision.

  **A `"fallback"` answer that CONSULTED the chain tier on this call reports the height this node's
  own held Chia peers ANNOUNCE, when one is known: `peak_height` is that height and `synced` is
  `true`.** With NO announced peer height it MUST report `synced: false` and `peak_height: null`,
  ...
  The bound is legitimate because the answer came from a full node reading the chain at its tip
  moments ago, while this node's own peers independently name that tip at the same moment. It is NOT
  the latched-replica case this section forbids: `initial_sync_complete` records a catch-up that once
  finished, so it can sit thousands of blocks behind, whereas a full node's current view is a read
  rather than a latch.

  **A `"fallback"` answer served from the chain tier's CACHE reports `synced: false` and
  `peak_height: null`.** A cached record was taken at an earlier, unrecorded moment — the rows are
  permanent by design, because a spent coin's record is immutable — so a current peer height would
  claim a currency nothing measured.

  This ordering fails toward WITHHELD structurally rather than by a guard: a node holding no chain
  peers has no announced height, so it keeps claiming nothing. It exists because with both fields
  hardcoded no read outside the wallet's own scope could EVER carry a warrant, which turned every
  consumer-side freshness guard into an unconditional refusal.
