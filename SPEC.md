...
token is NOT accepted for them. The exceptions are the wallet CHAIN READS the CALLER ADDRESSES — `control.wallet.balance`, `control.wallet.coins`,
`control.wallet.coinById` and `control.wallet.peak`
(`is_open_control_read`): each is a READ-ONLY
read of PUBLIC chain state with no custody, named by the caller, so each is served WITHOUT a token. They still route through `dispatch_control` (so they
stay in the control catalog and get their CLI verbs) but the server skips the token requirement.
The ACTIVITY CURSORS are NOT among them, despite also being chain reads: `control.wallet.arrivals`
and `control.wallet.sends` take only a cursor, so their answers volunteer this node's OWN watched
puzzle hashes and the money history behind them — the chain facts are public, the association
between this node and those addresses is not, and a token-less caller could replay the disclosed
addresses into the reads above. `control.wallet.sends` additionally says when the operator is
SPENDING. Membership of the open set turns on WHO NAMES THE ADDRESS, not on whether the data is on
chain. (An earlier revision of this section listed `control.wallet.arrivals` as open, which
contradicted `is_open_control_read` and the test pinning it; the code was correct.)
`control.wallet.broadcast` is NOT among them either: it puts bytes on the network, so it is token-gated like
...
| `control.wallet.arrivals` | `after_seq` (integer ≥ 0, default `0`), `limit` (integer, default `50`, CLAMPED to `1..=500`) | `arrivals` (`[{seq, coin_id, puzzle_hash, amount, asset_id, confirmed_height}]`, oldest first), `cursor` (the RESUME position: the last `seq` actually returned, or the caller's own `after_seq` on an empty page), `latest` (the newest position the ledger holds). A client MUST resume from `cursor` and MUST NOT resume from `latest`: `latest` is read after the page, so an arrival recorded in between sits above the page and below `latest`, and resuming from `latest` would step over it. `latest` exists for the first-run case only — a client with no stored cursor reads it and passes it back as `after_seq` to start from NOW rather than replaying the ledger as a burst of notifications. INCOMING FUNDS the node determined ARRIVED, since a cursor (dig_ecosystem#2548) — the question neither `.balance` (a total the user's own change also moves) nor `.coins` (no notion of "new") can answer. A row is written ONLY for a coin that is (a) CONFIRMED — `confirmed_height` is `NOT NULL` in the store, so a mempool sighting is unwritable, not merely unwritten; (b) confirmed STRICTLY ABOVE the wallet's arrival baseline, which is armed ONLY by the statement that records a COMPLETED address-history catch-up — the one caller that has demonstrably replayed everything — so a first catch-up announces nothing, and a point read against the fallback oracle, which replays nothing, cannot arm a baseline at all; (c) not already recorded, enforced by a `UNIQUE` coin id on disk, so a restart, a reconnect or a rebuilt replica re-announces nothing; and (d) NOT created by spending a coin this wallet holds, so the user's own change is never reported as a receipt. `amount` is a decimal STRING (the full `u64` range; a JSON number would round it). `asset_id` is `null` for native XCH and the CAT's hex TAIL otherwise — NEVER a ticker, because naming an asset the node did not attribute would assert a classification it cannot support; a coin whose asset is not yet determinable is HELD and re-examined, never announced as XCH. A reorg DELETES the arrivals above the fork with the coins they describe, and walks the baseline back; `seq` is `AUTOINCREMENT`, so a deleted row's position is never reused and a stored cursor cannot come to mean a different arrival. `arrivals: []` means the node consulted its OWN replica and nothing arrived since the cursor — it is NOT a claim that the replica is current (ask `control.wallet.syncStatus`), and a node that has never completed a catch-up has no baseline and reports empty forever. TOKEN-GATED, unlike the caller-addressed wallet reads: the caller supplies only a cursor, so the answer names this node OWN watched puzzle hashes and the receive history behind them. It touches only the local replica and has no oracle path, so it discloses nothing OFF-node -- but it discloses plenty to the CALLER, which is what the token stands between. `INVALID_PARAMS` on a negative `after_seq`; `WALLET_READ_FAILED` if the local ledger cannot be read. |
| `control.wallet.sends` | `after_seq` (integer >= 0, default `0`), `limit` (integer, default `50`, CLAMPED to `1..=500`) | `sends` (`[{seq, net_outflow, asset_id, confirmed_height}]`, oldest first), `cursor` (the RESUME position, same rule as `.arrivals`), `latest`. OUTGOING FUNDS the node determined LEFT the wallet, since a cursor (dig_ecosystem#2565). **`net_outflow` is what LEFT — the wallet own inputs at that height MINUS its own outputs — and is NEVER a spent coin amount**: spending a 9 XCH coin to send 1 XCH reports 1, not 9, because ~8 XCH returned as change. It INCLUDES any network fee and the node MUST NOT report the two separately: the replica holds only coins at puzzle hashes the wallet subscribed, so a send output to the RECIPIENT is structurally absent and no observation separates payment from fee. The figure therefore equals the drop in the balance this same node reports. A row is written ONLY for a height that is (a) CONFIRMED — heights are enumerated from `spent_height`, which is set only from a confirmed `CoinState`, so a mempool sighting belongs to no height; (b) STRICTLY ABOVE the wallet send baseline, armed ONLY by the statement recording a COMPLETED address-history catch-up (the catch-up subscribes with `include_spent`, so a first sync replays a whole spend history and must announce none of it); (c) not already recorded, enforced by `UNIQUE (confirmed_height)` on disk; and (d) fully ACCOUNTABLE — a height whose spend set is not purely plain XCH at a watched address is recorded as NOTHING, because a CAT change coin sits at a curried hash the peer sync path drops and scoring the difference would overstate the send by the entire change. AT MOST ONE row per confirmed height: a spend and its change are one transaction and one notification, and a multi-input spend is summed rather than counted per input. Two INDEPENDENT transactions confirmed in the same block therefore merge into one row with a correct total — there is no transaction id in the replica, so a height is the finest grouping that exists, and the residual errs toward FEWER notifications rather than a wrong figure. Recorded ONLY from the live sync path, never from the fallback-oracle point read, which has no height of its own and could see a removal before its change. A reorg DELETES the sends above the fork and walks the baseline back. TOKEN-GATED for the `.arrivals` reason plus one more: the answer says when the operator is SPENDING. `INVALID_PARAMS` on a negative `after_seq`; `WALLET_READ_FAILED` if the local ledger cannot be read. |
...
- `wallet balance <address> [--asset xch|dig]` → `control.wallet.balance`;
  `wallet coins <address> [--asset xch|dig]` → `control.wallet.coins`;
  `wallet coin-by-id <coin_id>` → `control.wallet.coinById`;
  `wallet arrivals [--after-seq <n>] [--limit <n>]` → `control.wallet.arrivals`;
  `wallet sends [--after-seq <n>] [--limit <n>]` → `control.wallet.sends`; `wallet peak` →
  `control.wallet.peak`; `wallet broadcast <signed_bundle_hex>` → `control.wallet.broadcast`. The
  caller-addressed reads (`balance`, `coins`, `coin-by-id`, `peak`) are OPEN and need no token; the
  activity cursors (`arrivals`, `sends`) are token-gated because their answers name this node's own
  addresses and activity rather than one the caller supplied; `broadcast` is token-gated like every
...
| -32040 | `WALLET_NO_CHAIN_SOURCE` | node | a wallet chain read (`control.wallet.balance`/`.coins`/`.coinById`/`.peak`) or `control.wallet.broadcast` had NO live chain source able to answer an arbitrary (non-wallet) address. Distinct from a truthful `0`. |
...
| -32042 | `WALLET_READ_FAILED` | node | `control.wallet.balance`/`.coins`/`.coinById`/`.peak` failed at the underlying DB / chain-source layer. On `.coinById` this INCLUDES a chain source that answered with a record for a DIFFERENT coin than the id asked for: a coin id is self-certifying (`SHA256(parent ‖ puzzle_hash ‖ amount)`), so a substituted record is a failed READ -- never that coin's record, and never `coin: null`. Distinct from `WALLET_NO_CHAIN_SOURCE` and `WALLET_NOT_SYNCED`. |
| -32043 | `WALLET_RATE_LIMITED` | node | `control.wallet.balance`/`.coins`/`.coinById` refused: the GLOBAL coinset-fallback rate bound is exhausted (too many arbitrary-address reads hit the expensive fallback in a short window). Defense-in-depth against an open-read amplification/oracle sweep; back off and retry. The cheap local-DB fast path is never gated. |
