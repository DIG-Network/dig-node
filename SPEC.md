...
`control.wallet.peak` and `control.wallet.coinById` (`is_open_control_read`): each is a READ-ONLY read of PUBLIC chain state with
...
| `control.wallet.coinById` | `coin_id` (64 lowercase-hex; an optional `0x` prefix is tolerated on input and stripped — it is NEVER echoed in the response) | `coin` (a record or `null`), `source` (always `"fallback"`), `synced` (always `false`), `peak_height` (always `null`). The record has the SAME shape as a `control.wallet.coins` element (`{coin_id, asset, amount, parent_coin_info, puzzle_hash, created_height, spent_height}`) and MAY be SPENT — a spent coin carries a non-null `spent_height`. `source`, `synced` and `peak_height` are ALWAYS `"fallback"`, `false` and `null` respectively because this read DELIBERATELY bypasses the local replica: a replica miss means "this node does not watch that coin", not "the coin does not exist", so the node MUST consult an external chain source rather than returning `null` on a local miss. `coin: null` MUST mean a chain WAS consulted and provably has no such coin; every way of FAILING to consult one is a DISTINCT error (`WALLET_NO_CHAIN_SOURCE`/`WALLET_READ_FAILED`/`WALLET_RATE_LIMITED`, §10), NEVER a null coin — `null` on a read failure would tell a caller the coin does not exist when the truth is the node could not check. A malformed `coin_id` is `INVALID_PARAMS`, refused BEFORE any network call. OPEN read (no token). |
...
- `wallet balance <address> [--asset xch|dig]` → `control.wallet.balance`;
  ...
  `control.wallet.peak`; `wallet coin-by-id <coin_id>` → `control.wallet.coinById`;
  `wallet broadcast <signed_bundle_hex>` → `control.wallet.broadcast`. The
  first four are the OPEN chain reads and need no token; `broadcast` is token-gated like every
...
| -32040 | `WALLET_NO_CHAIN_SOURCE` | node | a wallet chain read (`control.wallet.balance`/`.coins`/`.peak`/`.coinById`) or `control.wallet.broadcast` had NO live chain source able to answer an arbitrary (non-wallet) address. Distinct from a truthful `0`. |
...
| -32042 | `WALLET_READ_FAILED` | node | `control.wallet.balance`/`.coins`/`.coinById` failed at the underlying DB / chain-source layer. Distinct from `WALLET_NO_CHAIN_SOURCE` and `WALLET_NOT_SYNCED`. |
