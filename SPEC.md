...
| `control.chiaPeers.add` | `ip` (the peer's IP address; the standard full-node port is assumed) | `{added: true, ip, port, corroboration_bypassed: true, notice}`. TRUSTS a Chia full node: it writes the `user_managed` peer row that is the ONLY way to reach `PeerTrust::Operator`, the trust level whose answers may drive catch-up, rollback and the `initial_sync_complete` flag WITHOUT a quorum. Every other peer is `Discovered` and must be corroborated by independently chosen peers first (§18.16). `corroboration_bypassed` is ALWAYS `true` and `notice` carries the cost as a sentence, so a client quotes the node's own words rather than keeping a second copy that can drift; a surface offering this method MUST state that cost where the person adds the peer. Idempotent — re-adding a known peer succeeds and un-bans it. A node MUST serve this from the SAME peer store its wallet replica consults; a second peer list would let the trusted set and the consulted set drift apart silently. `INVALID_PARAMS` on a missing or blank `ip`, refused before any write — a blank address is a storable row nobody can dial or delete. TOKEN-GATED (not an open read). |
| `control.chiaPeers.list` | — | `{peers: [{ip, port, peak_height, user_managed}]}` — every tracked, non-banned Chia peer, TRUSTED AND DISCOVERED ALIKE. `user_managed` is what tells them apart and MUST be reported rather than filtered on: a list showing only the trusted set would let a person conclude the node talks to nobody else. `peak_height` is `0` where the node holds no telemetry for that peer yet and MUST NOT be fabricated. TOKEN-GATED — the list discloses which peers this node associates with. |
| `control.chiaPeers.remove` | `ip`, optional `ban` (bool, default `false`) | `{removed: true, ip, banned}`. Stops trusting a peer, RESTORING corroboration for it. `ban: true` keeps the peer excluded so discovery cannot re-add it; `false` merely forgets the row. Idempotent — removing an absent peer succeeds. `INVALID_PARAMS` on a missing or blank `ip`. TOKEN-GATED. |
...
- `chia-peers` / `chia-peers list` → `control.chiaPeers.list` — the tracked CHIA full-node peers (a
  different network from `peers`, which is DIG gossip). The human view counts the TRUSTED peers
  separately from the total, because those are the only ones that can move the replica on their own
  word, and labels each row `trusted (you added it)` or `discovered (must be corroborated)`. An empty
  list SAYS so and names the verb that fills it. `--json` emits the result verbatim.
- `chia-peers add <ip>` → `control.chiaPeers.add` — TRUST a Chia full node. The command MUST state
  the corroboration it costs at the moment it is granted: `--help` explains that this node otherwise
  believes a chain answer only once several independently-chosen peers agree, and that a peer added
  here can advance, roll back or complete the wallet replica alone; the success line repeats the
  node's own `notice`, falling back to an equivalent sentence against a node too old to send one. A
  bare `chia-peers` with no sub-action LISTS rather than adds — a default must never be the act that
  costs something.
- `chia-peers remove <ip> [--ban]` → `control.chiaPeers.remove` — stop trusting a peer. The output
  distinguishes forgetting from banning, because only the latter stops discovery re-adding it.
