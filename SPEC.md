...
token is NOT accepted for them. The exceptions are the wallet CHAIN READS — `control.wallet.balance`, `control.wallet.coins` and `control.wallet.peak` (`is_open_control_read`) — each of which needs only PUBLIC chain state. `control.wallet.broadcast` is NOT among them: it puts bytes on the network, so it is token-gated like every other mutation, and an `UNAUTHORIZED` from it means exactly that (remedy: the token) where the same code on an open read can only come from a node too old to serve it (remedy: an upgrade):
...
- **The chain transport is attached on EVERY install (dig_ecosystem#2376).** The wallet's chain
  ...
  hand it straight back for relay. While the flag is off, a bundle spending a coin at ANY puzzle
  hash the node custodies a key for is refused with `WALLET_NODE_SPEND_DISABLED` (§10) — a single
  such spend refuses the whole bundle. The custodied set is the union of the loaded signer, every
  signer this process has loaded (the §18.24 per-transaction grant is already gone by push time),
  and the custody manifest's addresses (readable while locked). Merely WATCHED puzzle hashes are NOT
  in it: the node holds no key for those, and refusing them would block a legitimate third-party
  push. Without this check the flag would be decorative on the one path that matters.
