...
`control.wallet.coinById`, `control.wallet.arrivals` and `control.wallet.peak`
(`is_open_control_read`): each is a READ-ONLY
read of PUBLIC chain state with no custody, so each is served WITHOUT a token. They still route through `dispatch_control` (so they
...
- `wallet balance <address> [--asset xch|dig]` → `control.wallet.balance`;
  ...
  first five are the OPEN chain reads and need no token; `broadcast` is token-gated like every
