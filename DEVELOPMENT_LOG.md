...
## The node runs ONE mTLS identity across ALL listeners — the gossip pool MUST reuse the NodeCert (#1532)

The node has two in-process TLS listeners: the peer-RPC server (:9444) and the dig-gossip connected
pool (:9445). The advertised/registered/pinned `peer_id` (`peer_id = SHA-256(TLS SPKI DER)`) comes
from the persistent CA-signed `NodeCert` under `node_cert_dir()` (`peer-net/identity/node.crt`+`.key`).
dig-gossip's `GossipConfig` loads its listener cert from `cert_path`/`key_path` via
`dig_peer_protocol::load_ssl_cert`, and if those files are absent it mints its OWN throwaway
`ChiaCertificate` — a DIFFERENT SPKI → a DIFFERENT `peer_id`. Pointing the pool at its own
`peer-net/node.cert` (the original wiring) therefore made the pool listener present an identity that
did NOT match the advertised one, so every relayed/direct dial failed CLOSED with
...
`dig_gossip::peer_id_from_tls_spki_der(identity.spki_der())`. RULE: any new node listener presents the
NodeCert; never let a sub-component mint its own transport identity. Regression:
