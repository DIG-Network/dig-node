...
//! sync with what the EMBEDDED read path (`digstore_node::handle_rpc`) actually
//! resolves.
//!
...
//! DISPATCHING each catalogued read method through the real `digstore_node::handle_rpc`
//! and asserting that:
//!
...
//!     is NOT resolved by the read path (returns `-32601`, which is the dig-node
//!     shell's cue to relay it to the upstream).
//!
//! If a future read-path rev moves a method between local and passthrough — or the
...
use dig_node::meta::{self, ErrorCode};
use digstore_node::{handle_rpc, Node};
use serde_json::json;
use std::sync::Arc;
...
/// The dig-node-shell methods that `digstore_node::handle_rpc` never sees (they are
...
async fn local_methods_are_resolved_by_dig_node() {
    ...
    for m in meta::methods() {
        ...
        assert_ne!(
            ...
            "catalogue marks {} served=local, but the read path returned -32601 \
...
async fn passthrough_methods_are_not_resolved_by_dig_node() {
    ...
    for m in meta::methods() {
        ...
        assert_eq!(
            code,
            Some(METHOD_NOT_FOUND),
            "catalogue marks {} served=passthrough, but the read path did NOT return \
